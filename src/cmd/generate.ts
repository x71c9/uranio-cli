...
let generate_types = default_params as Params;
...
async function _run_generate(){
	...
	return await new Promise((resolve, reject) => {
		...
		const generate_path_js = `${generate_types.root}/${defaults.folder}/generate/dist/generate.js${generate_arg1}${generate_arg4}`;
		const flags = ``;
...
function _compile_generate(){
	...
	esbuild.buildSync({
		entryPoints: [`${generate_types.root}/${defaults.folder}/generate/src/generate.ts`],
		outfile: `${generate_types.root}/${defaults.folder}/generate/dist/generate.js`,
		bundle: true,
		platform: 'node',
		sourcemap: false,
		// minify: true
...
function _init_types(params:Partial<Params>, must_init=true)
	...
	generate_types = merge_params(params);
