...
async function _remove_dockers()
	...
	await docker_db_stop(deinit_params, dotenv.URN_DB_TYPE as DB);
	await docker_db_remove(deinit_params, dotenv.URN_DB_TYPE as DB);
	await docker_remove(deinit_params);
	await docker_unbuild(deinit_params);
}
