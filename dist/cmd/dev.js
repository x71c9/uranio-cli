...
function _save_nuxt_config(ts_ast) {
    const nuxt_config_path = `${dev_params.root}/${defaults_1.defaults.folder}/client/nuxt.config.js`;
...
function _update_nuxt_proxy(ts_ast, service_url) {
    const all = ts_ast.program.body;
    for (const node of all) {
        if (node.type === 'ExportDefaultDeclaration') {
            const obj_declaration = node.declaration;
...
function _replace_proxy(prop, service_url) {
...
function _add_uranio_proxy(prop, service_url) {
    const b = recast.types.builders;
    if (prop.value.type === 'ObjectExpression') { // {'/some/path': {...}}
        const props = prop.value.properties;
        const uranio_prop = b.objectProperty(b.stringLiteral('/uranio/api'), _uranio_proxy_object_expression(service_url));
        props.push(uranio_prop);
    }
}
function _uranio_proxy_object_expression(service_url) {
    const b = recast.types.builders;
    return b.objectExpression([
        b.objectProperty(b.identifier('target'), b.stringLiteral(service_url)),
        b.objectProperty(b.identifier('pathRewrite'), b.objectExpression([
            b.objectProperty(b.stringLiteral('^/uranio/api'), b.stringLiteral(''))
        ]))
    ]);
}
function _nuxt_server_props(domain, port) {
    const host = (domain === 'localhost') ? '0.0.0.0' : domain;
    const b = recast.types.builders;
    return [
        b.objectProperty(b.identifier('host'), b.stringLiteral(host)),
        b.objectProperty(b.identifier('port'), b.numericLiteral(port))
    ];
}
function _add_server_to_nuxt_properties(obj_decl, domain, port) {
    const b = recast.types.builders;
    const proxy_prop = b.objectProperty(b.identifier('server'), b.objectExpression(_nuxt_server_props(domain, port)));
    obj_decl.properties.push(proxy_prop);
}
function _add_proxy_to_properties(obj_decl, service_url) {
    const b = recast.types.builders;
    const proxy_prop = b.objectProperty(b.identifier('proxy'), b.objectExpression([
...
function _fix_mongodb_saslprep_requirement() {
    ...
    util_instance.fs.copy_file(`${saslprep_module_dir}/${saslprep_filename}`, `${dist_dir}/server/${saslprep_filename}`);
}
